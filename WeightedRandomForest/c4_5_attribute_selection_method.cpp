...
void C4_5AttributeSelectionMethod::ExecuteSelection() {
	...
	for (iter_info_gain = mapper_info_gain.begin();iter_info_gain != mapper_info_gain.end(); ++iter_info_gain) {
	...
	if (!is_set_attribute) {
		attribute = mapper_info_gain.begin()->first;
	}
	...
	this->set_attribute_info_gain_(mapper_info_gain[attribute]);
