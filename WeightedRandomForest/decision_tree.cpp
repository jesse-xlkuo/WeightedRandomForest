...
DecisionTree::DecisionTree(TrainingSet* training_set){
...
Node* DecisionTree::GenerateDecisionTreeByC4_5(vector<int> training_set_index,vector<int> attribute_list){
	if(training_set_index.size()==0){
		...
		C4_5AttributeSelectionMethod method(this->training_set_,training_set_index,attribute_list);
			...
			if(this->training_set_->GetAttributeType(attribute)==DISCRETE){
				...
				for(iter=mapper_split_result.begin();iter!=mapper_split_result.end();++iter){
					if(iter->second.size()==0){  // maybe this if branch can be deleted
				    	...
				    	node->set_child_node_(iter->first,GenerateDecisionTreeByC4_5(iter->second,attribute_list));
				    }
				}
				return node;
			}
...
//		map<int,Node*> mapper=((InternalNode*)(decision_tree))->get_all_child_nodes_();// map<int,Node*> is discarded
//
